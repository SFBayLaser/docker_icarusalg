...
RUN mkdir larsoft && \
  ...
  mkdir products && \
  ./pullProducts products/ slf7 larsoftobj-v09_27_00 e20 prof && \
  ./pullProducts products/ slf7 larsoftobj-v09_27_00 e20 prof && \
  rm *tar.bz2
...
RUN cd / && \
  ...
  setup larsoftobj v09_27_00 -q e20:prof && \
...
RUN cd / && \
  ...
  mkdir icarusalg && \
  cd icarusalg && \
  mrb newDev -v v09_65_03 -q e20:prof && \
  source localProducts_*/setup && \
  cd srcs/ && \
  mrb g --tag v09_65_03 icarusalg && \
  cd ../build* && \
  mrbsetenv && \
  mrb i
